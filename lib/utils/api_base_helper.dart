  ...
  Future<dynamic> postMultipart(String url,
    ...
    try {
      ...
      var request = customHttp.MultipartRequest(
        isPut ? customHttp.HttpMethod.PUT.asString : customHttp.HttpMethod.POST.asString,
        uri,
      );
      ...
      if (files.isNotEmpty) {
        for (var element in files) {
          request.files.add(await customHttp.MultipartFile.fromPath(key, element));
          print(element);
        }
      }
  ...
  Future<bool> shouldAttemptRetryOnResponse(
    ...
    if (response.statusCode == 401 && !response.request!.url.toString().contains(ApiConstants.login)) {
      ...
      try {
        ...
        final response = await flutter_http.post(uri, headers: headers,body: jsonEncode({'random_string':refreshToken}));
